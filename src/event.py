...
class Event:
    ...
    def set_effect_params(self, time_slot, interface):
        ...
        return (
            self.impact_params[time_slot]['plr_mean'],
            self.impact_params[time_slot]['latency_mean'],
            self.impact_params[time_slot]["latency_std_mean"],
            self.impact_params[time_slot]['distribution'],
            interface
        )
...
def create_event_params(event_params_file, perfect_stable: bool = False):
    ...
    df.loc[
        (df["plr_mean"] < stable["plr_mean"]) & (df["bin"].abs() > 2), "plr_mean"
    ] = stable["plr_mean"]
    df.loc[
        (df["latency_mean"] < stable["latency_mean"]) & (df["bin"].abs() > 2),
        "latency_mean",
    ] = stable["latency_mean"]
