...
class Profile_Based_Controller(Controller):
    def __init__(self, event_params_file, db, interface, rate_mbit=1000, burst_mbit=100, latency_ms=5, perfect_stable: bool = False):
    ...
    def run(self):
        params = self.event_dict['Stable'].set_effect_params(0.0, self.interface)
        self.run_netem_cmd(*params)
        ...
        for r in self.config_sched_df.itertuples():
            ...
            params = self.event_dict[r.type].set_effect_params(r.bin, self.interface)
            self.run_netem_cmd(*params)
